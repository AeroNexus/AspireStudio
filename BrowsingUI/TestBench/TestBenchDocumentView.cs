...
using Aspire.UiToolbox.Controls;
...
{
  public partial class TestBenchView : StudioDocumentView//, IPrinterSupport
  {
...
<B>DeviceUnderTest</B>: An optional attribute applied to a test method to specify which device is being tested. Each method can have multiple [DeviceUnderTest] attributes. Specify IsSpecifiedAsDeviceKind = true to indicate that the Name refers to a KIND.<p>
    ...
    private TestBenchDocument myDoc;

    public override StudioDocument Document
    {
      get
      {
      ...
      set
      {
        myDoc = mDocument as TestBenchDocument;
        if (myDoc != null && myDoc.ScriptObject != null)
        {
          if (myDoc.ScriptObject.CompiledObject is IDisposable)
          {
            (myDoc.ScriptObject.CompiledObject as IDisposable).Dispose();
          }

          myDoc.PropertyChanged -= new PropertyChangedEventHandler(Document_PropertyChanged);

          myDoc.ScriptObject.ScriptCompiledSuccessfully -= new EventHandler(ScriptObject_ScriptCompiledSuccessfully);
        }
        ...
        if (myDoc != null)
        {
          editor.ScriptObject = myDoc.ScriptObject;

          if (!string.IsNullOrEmpty(myDoc.Caption))
          {
            this.Text = myDoc.Caption;
          }

          myDoc.PropertyChanged += new PropertyChangedEventHandler(Document_PropertyChanged);

          if (myDoc.ScriptObject != null)
          {
            myDoc.ScriptObject.ScriptCompiledSuccessfully += new EventHandler(ScriptObject_ScriptCompiledSuccessfully);
            try
            {
              myDoc.ScriptObject.Compile();
            }
            catch (Exception ex)
            {
    ...
    {
      if (!string.IsNullOrEmpty((Document as TestBenchDocument).Caption))
      {
        this.Text = (Document as TestBenchDocument).Caption;
      }
    ...
    {
      List<DeviceUnderTestAttribute> deviceNames = new List<DeviceUnderTestAttribute>(method.GetCustomAttributes(typeof(DeviceUnderTestAttribute), false).
      ...
      else
      {
        deviceNames.ForEach(attr =>
        {
          if (attr.Name == "*")
          {
            var list = PnPBrowsers.AllComponents.Select(c => c);
          ...
          else if (attr.IsSpecifiedAsDeviceKind)
          {
            ...
            var list = PnPBrowsers.AllComponents.Where(
          ...
          else     // otherwise, a device name is specified
          {
            ...
            AspireComponent comp = PnPBrowsers.AllComponent(deviceName);
    ...
    private void LoadTestMethods()
    {
      ...
      System.Type type = myDoc.ScriptObject.CompiledObject.GetType();
      System.Reflection.MethodInfo[] methods = type.GetMethods();

      if (methods != null)
      {
        methods.Where(method => method.GetCustomAttributes(typeof(TestMethodAttribute), false).Length > 0).
            ForEach(method => CreateNodesForMethod(method));

        m_SetupMethod = methods.FirstOrDefault(method =>
            method.GetCustomAttributes(typeof(TestInitializeAttribute), false).Length > 0);

        m_TearDownMethod = methods.FirstOrDefault(method =>
            method.GetCustomAttributes(typeof(TestCleanupAttribute), false).Length > 0);

        m_TestFixtureSetupMethod = methods.FirstOrDefault(method =>
            method.GetCustomAttributes(typeof(ClassInitializeAttribute), false).Length > 0);

        m_TestFixtureTearDownMethod = methods.FirstOrDefault(method =>
            method.GetCustomAttributes(typeof(ClassCleanupAttribute), false).Length > 0);
      }
    ...
    private void PerformTestTearDown()
    {
      ...
      if (m_TearDownMethod != null)
      {
        try
        {
          ...
          m_TearDownMethod.Invoke(myDoc.ScriptObject.CompiledObject, null);
        ...
        catch (Exception ex)
        {
          ...
          Log.WriteLine(Log.Severity.Error,"Error calling TearDown", ex);
        }
      }
    }
    ...
    private bool PerformTestFixtureSetUp()
    {
      if (m_TestFixtureSetupMethod != null)
      {
        try
        {
          ...
          m_TestFixtureSetupMethod.Invoke(myDoc.ScriptObject.CompiledObject, null);
        ...
        catch (Exception ex)
        {
          if (ex.InnerException != null && ex.InnerException.GetType() == typeof(AssertFailedException))
          {
            Log.WriteLine(Log.Severity.Error, "TestBench", "TestFixtureSetUp failed:{1}{0}",
                ex.InnerException.Message, System.Environment.NewLine);
          }
          else
          {
            Log.ReportException(ex, "TestBenchDocumentWindow", "TestFixtureSetUp failed");
          }

          Log.WriteLine(Log.Severity.Error, "Error calling TestFixtureSetUp method");
    ...
    private void PerformTestFixtureTearDown()
    {
      if (m_TestFixtureTearDownMethod != null)
      {
        try
        {
          ...
          m_TestFixtureTearDownMethod.Invoke(myDoc.ScriptObject.CompiledObject, null);
        ...
        catch (Exception ex)
        {
          ...
          Log.WriteLine(Log.Severity.Error, "Error calling TestClassSetUp method: {0}", ex);
        }
      }
    }
    ...
    private void ExecuteMethodForNode(Node deviceNode, Node node)
    {
      ...
      TestBenchScriptObject scriptObject = (myDoc.ScriptObject.CompiledObject as TestBenchScriptObject);
      ...
      if (method != null && method.MethodInfo != null)
      {
        try
        {
          ...
          {
            m_SetupMethod.Invoke(myDoc.ScriptObject.CompiledObject, null);
          }

          m_Results.m_TestsRun++;

          // Call the test method
          method.MethodInfo.Invoke(myDoc.ScriptObject.CompiledObject, null);
    ...
    {
      if (nodesToRun.Count() > 0 && myDoc != null && myDoc.ScriptObject != null && myDoc.ScriptObject.CompiledObject != null)
      {
        ...
        Log.WriteLine(Log.Severity.Info, m_Results.ToString());
    ...
    private void button1_Click(object sender, EventArgs e)
    {
      ...
      {
        Log.ReportException(ex, "TestBench: Error running tests");
      }
      finally
      {
        lblRunResults.Text = m_Results.ToString();
    ...
    private void mnuSaveResults_Click(object sender, EventArgs e)
    {
      ...
      dlg.InitialDirectory = Scenario.Directory;
    ...
    private void allCheckedNodesToolStripMenuItem_Click(object sender, EventArgs e)
    {
      ...
      if (dlg.ShowDialog(this) == DialogResult.OK)
      {
        ...
        FileUtilities.WriteToXml(resultsForExport, dlg.FileName, true);
      }
    }
    ...
    private void ExportTestResultsForSelectedNode(string filename)
    {
      ...
      FileUtilities.WriteToXml(resultsForExport, filename, true);
    }
    ...
    //            try
    //            {
    //                client = new AeroNexus.Aspire.BrowsingUserInterface.RepositoryWebService.RepositoryWebServiceSoapClient();
    //            }
      ...
      if (e.Data.GetDataPresent(typeof(string)))
      {
        if (editor.Editor.SelectedView.Selection != null && editor.Editor.SelectedView.Selection.Length == 0)
        {
          ...
          editor.Editor.SelectedView.Selection.SelectRange
              (editor.Editor.SelectedView.LocationToOffset(p, SyntaxEditor.LocationToPositionAlgorithm.BestFit), 0);
        }
